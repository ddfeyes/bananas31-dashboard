    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          borderColor: 'rgba(255,255,255,0.08)',
          borderWidth: 1,
          callbacks: { label: ctx => ' OI: ' + fmtK(ctx.raw) },
        },
      },
      scales: {
        x: {
...
function updateLastPrice(price, change) {
  ...
  if (changeEl) {
    const sign = change >= 0 ? '+' : '';
    changeEl.textContent = sign + fmt(change, 4);
...
async function renderOiChart() {
  ...
  if (el) {
    el.innerHTML = `
      <span class="oi-stat">OI <span style="color:${pctColor(pct)}">${fmtK(last)}</span></span>
...
// ── Render: Trade Tape ────────────────────────────────────────────────────────
// Track last seen trade ID to avoid duplicates
...
async function renderTradeTape() {
  ...
  // Find new trades (newer than last seen)
  ...
  const rows = newTrades.map(t => {
    ...
    return `<div class="trade-row${whale}">
        ...
        <span class="trade-price">${fmt(price, 4)}</span>
  ...
  while (children.length > TRADE_MAX) {
    tape.removeChild(children[children.length - 1]);
  }
}
...
async function renderPhase() {
  ...
  // API returns: {regime, phase, phase_confidence, score, action, weights, symbol}
...
function connectAlerts() {
  ...
  wsAlerts.onmessage = (evt) => {
    try {
      ...
      showAlert(msg.description || msg.message || JSON.stringify(msg));
    } catch (_) {}
  };
